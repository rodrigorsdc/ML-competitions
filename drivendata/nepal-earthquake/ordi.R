...
remove_predictors <- c(1, 39, 38, 37, 36, 35, 34, 33)
...
# quake2 = quake2[sample(nrow(quake2), 25000),]
# quake3 = quake3[sample(nrow(quake3), 25000),]

# quake = rbind(quake1, quake2, quake3)
quake = quake[sample(nrow(quake)),]
quake$damage_grade = factor(quake$damage_grade, ordered=TRUE)
print(nrow(quake))
...
model <- polr(damage_grade ~ ., data=quake, subset=train)
## ordi.fit <- stepAIC(model, k = log(nrow(quake)))
if (TEST == TRUE) {
    ...
    f1_micro(quake[-train,]$damage_grade, pred)
}
